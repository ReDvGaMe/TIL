...
- 인접 행렬 표현  
    ...
    - |V| x |V| 크기의 2차원 배열을 사용하기 때문에, 실제 간선의 개수와 관계 없이 항상 O(|V|^2) 크기의 공간을 사용
...
- 인접 리스트 표현
    ...
    (간혹 간선의 수가 |V|^2에 비해 훨씬 적은 그래프를 희소 그래프(sparse graph),  
    반대로 간선의 수가 거의 |V|^2에 비해 간선의 수가 훨씬 적은 밀집 그래프(dense pragh) 라고 함  
...
```
그래프에서 모든 정점들이 간선을 통해 연결되어 있다는 보장이 없기 때문에, dfs()만으로는 모든 정점을 순서대로 발견한다는 목적에 부합하지 않음  

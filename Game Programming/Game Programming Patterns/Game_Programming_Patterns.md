...
화면에 색을 뿌�� 때 프레임버퍼로부터 한 바이트씩 색깔 값을 읽어옴  
...
Superpower를 상속받은 초능력 클래스에서 사운드, 시각 이펙트. AI와��� 상호작용, 다른 게임 개체의 생성과 파괴, 물리 작용 같은 모든 일을 할 수 있다고 할 때,  
...
- 메시지를 전달하는 방식  
  ...
  컴포넌���가 컨테이너에 메시지를 보내면, 컨테이너는 자기에게 있는 모든 컴포넌트에 이를 전파  
...
#### 16.6 예제 코드
이벤트 큐에서 다룬 오디오 시스템  
이번에는 서비스 중개자를 통해서 다른 코드에 오디오 시스템을 제공하게 할 것임
...
```
이 클래스가 추상 인터페이스일 뿐 실제 구현은 바인딩 되어있지 않음

- 서비스 중개자  
오디오 인터페이스만으로는 아무것도 할 수 없음, 구체 클래스를 구현해야 함  
```
...
```
정적 함수인 getAudio()가 중개 역할을 함  
어디에서나 부를 수 있는 getAudio 함수는 다음과 같이 Audio 서비스 인스턴스를 반환  

```
Audio *audio = Locator::getAudio();
audio->playSound(VERY_LOUD_BANG);
```
Locator가 오디오 서비스를 '등록하는'방법은 단순함  
어디에서 getAudio를 호출하기 전에 먼저 서비스 제공자를 외부 코드에서 등록해주면 됨  
게임이 시작될 때 다음과 같은 코드를 실행

```
ConsoleAudio *audio = new ConsoleAudio();
Locator::provide(audio);
```
playSound()를 호출하는 쪽에서는 Audio라는 추상 인터페이스만 알 뿐 ConsoleAudio라는 구체 클래스에 대해서는 전혀 모른다는 게 핵심  
Locator 클래스 역시 서비스 제공자의 구체 클래스와는 커플링되지 않음  
어떤 구체 클래스가 실제로 사용되는지는 서비스를 제공하는 초기화 코드에서만 알 수 있음  

디커플링은 이뿐만이 아니라, Audio 인터페이스도 자기가 서비스 중개자를 통해서 여기저기로부터 접근된다는 사실을 모름  
Audio 인터페이스만 놓고 보면 일반적인 추상 사우이 클래스임  
즉,  꼭 서비스 중개자 패턴용으로 만들지 않은 기존 크랠스에도 이 패턴을 적용할 수 있음  
이런 점은 '서비스'를 제공하는 클래스의 형태 자체에 영향을 미치는 싱글턴 패턴과 정반대

- 널 서비스
위의 코드는 단순하면서도 충분히 유연함  
다만, 한 가지 큰 단점이 있음  
만약 서비스 제공자가 서비스를 등록하기 전에 서비스를 사용하려고 시도하면 NULL을 반환함  
이때 호출하는 쪽에서 NULL 검사를 하지 않으면 크래시 됨  

이럴 때 써먹을 수 있는 '널 객체' 디자인 패턴이 있음  
핵심은 객체를 찾지 못하거나 만들지 못해 NULL을 반환해야 할 때, 대신 같은 인터페이스를 구현한 특수한 객체를 반환하는 데 있음  
이런 특수 객체에는 아무런 구현이 되어 있지 않지만, 객체를 반환받는 쪽에서는 '진짜' 객체를 받은 것처럼 안전하게 작업을 진행할 수 있음  

널 객체 패턴을 사용하려면 다음과 같이 '널' 서비스 제공자를 정의
```
...
NullAudio는 Audio 서비스 인터페이스를 상속받지만 아무 기능도 하지 않음  
이제 Locator 클래스를 다음과 같이 바꿈
...
```
호출하는 쪽에서는 '진짜' 서비스가 준비되어 있는지를 신경 쓰지 않아도 되고 NULL 반환 처리도 필요 없음  

널 서비스는 의도적으로 특정 서비스를 못 찾게 하고 싶을 때에도 유용함
어떤 시스템을 잠시 못 쓰게 하고 싶다면 서비스를 그냥 등록함  

- 로그 데커레이터  
서비스 중개자 패턴을 활용한 '데커레이션으로 감싼 서비스'  

개발하다 보면, 게임 코드 안에서 어떤 일이 벌어지는지를 알기 위해 원하는 이벤트에 간단하게 로그를 설치해야 할 때가 있음  
AI 프로그래머라면 개체가 AI 상태를 언제 바꾸는지를 알고 싶을 수 있음  
사운드 프로그래머라면 모든 사운드가 출력될 때 로그를 남겨서 정확한 순서로 실행되고 있는지를 확인하고 싶을 수 있음  

이럴 때 보통 log() 함수를 코드 여기저기에 집어 넣는데, 이러다 보면 로그가 **너무 많아지는** 문제가 생김  

원하는 로그만 켰다 껏다 할 수 있고, 최종 빌드에는 로그를 전부 제거할 수 있다면 가장 이상적임  

조건적으로 로그를 남기고 싶은 시스템이 서비스로 노출되어 있따면, 데커레이터 패턴으로 이를 해결 가능  
...
LoggedAudio 클래스는 다른 오디오 서비스 제공자를 래핑하는 동시에 같은 인터페이스를 상속 받음  
실제 기능 요청은 내부에 있는 서비스에 전달하고, 대신 사운드가 호출될 때마다 로그를 남김  
오디오 로그 기능을 키려면 다음과 같이 하면 됨
...
#### 16.7 디자인 결정
구현은 다음과 같은 핵심적인 질문들을 고려해 다양하게 달라질 수 있음
#### 서비스는 어떻게 등록되는가?
- 외부 코드에서 등록  
  예제 코드에서 서비스를 등록하는 방식이 이에 해당  
  가장 일반적으로 사용되는 방법
  - 빠르고 간단  
    getAudio()는 단순히 포인터만 반환하면 됨  
    이런 함수는 컴파일러에서 인라인시킬 수 있기 때문에, 성능 손해 없이 깔끔한 추상 계층을 둘 수 있음  
  - 서비스 제공자를 어떻게 만들지 제어 가능      
  - 게임 실행 도중에 서비스를 교체할 수 있음
  - 서비스 중개자가 외부 코드에 의존한다는 단점
...
- 컴파일할 때 바인딩  
  ...
  ```
  - 빠름  
    모든 작업이 컴파일할 때 끝나기 때문에 런타임에 따로 할 일이 없음  

  - 서비스는 항상 사용 가능  
    서비스 중개자는 컴파일할 때 선택된 서비스를 소유하기 때문에, 컴파일만 된다면 서비스가 준비되어 있지 않을까 봐 걱정할 필요가 없음  

  - 서비스를 쉽게 변경할 수 없음
  
- 런타임에 설정 값 읽기  
  서비스 제공자 설정 파일을 로딩한 뒤에, 리플렉션으로 원하는 서비스 제공자 클래스 객체를 런타임에 생성함
  - 다시 컴파일하지 않고도 서비스를 교체할 수 있음  
  - 프로그래머가 아니어도 서비스를 바꿀 수 있음
  - 등록 과정을 코드에서 완전히 빼냈기 때문에 하나의 코드로 여러 설정을 동시에 지원할 수 있음  
    앱을 하나 배포한 후에 설정 값만 고쳐도 여러 다른 서버 환경에서 사용할 수 있기 때문에 기업형 웹 소프트웨어에서 이 방식을 많이 사용함  
    요즘은 모바일 게임이 많이 나오면서 이 방식이 적합할 수 있음  
...
- 사용자가 알아서 처리하게 함  
  - 실패했을 때 어떻게 처리할지를 사용자 쪽에서 정할 수 있음
  - 서비스 사용자 쪽에서 실패를 처리해야 함
...
- 게임을 멈춤  
  ...
  ```
  단언문을 넣음으로써 '서비스를 찾지 못하는 것은 중개자의 버그다'라는 걸 분명히 할 수 있음
  - 사용자 측에서는 서비스가 없는 경우를 처리하지 않아도 됨
  - 서비스를 찾지 못하면 게임이 중단됨

- 널 서비스를 반환  
- 외부 코드에서는 서비스가 없는 경우를 처리하지 않아도 됨
- 서비스를 사용할 수 없을 때에도 게임을 계속 진행할 수 있음
  장점이다 단점  
  어떤 서비스가 아직 준비되어 있지 않아도 게임을 계속 진행할 수 있다는 점은 장점  
  규모가 큰 팀에서 작업 중인 기능이 아직 구현되지 않은 다른 시스템에 의존할 때 특히 유리  
  단점은 의도치 않게 서비스를 찾지 못할 때에도 디버깅하기가 쉽지 않다는 점  

셋 중에서는 단언문을 거는 형태가 많이 사용됨  
게임이 출시될 쯤이면 서비스를 찾지 못할 가능성이 굉장히 적음  

규모가 큰 팀이라면 널 서비스를 추천  
서비스를 사용하지 못하는 동안에도 멈추지 않고 계속 개발할 수 있음  

#### 서비스의 범위는 어떻게 잡을 것인가?
지금까지는 어디에서나 중개자를 통해서 서비스를 접근할 수 있다고 가정해옴  
이게 서비스 중개자 패턴에서 일반적이기는 하나, 다음과 같이 특정 클래스 및 그 클래스의 하위 클래스에만 접근을 제한할 수도 있음
...
protected:
  // 서비스를 제공하는 정적 메서드는 하위 클래스에서만 접근할 수 있음
  static Audio& getAudio() { return *service_; }
...
```
이렇게 하면 Base 클래스를 상속받는 클래스에서만 오디오 서비스에 접근 가능  

- 전역에서 접근 가능한는 경우
  - 전체 코드에서 같은 서비스를 쓰도록 함  
  - 언제 어디에서 서비스가 사용되는지를 제어할 수 없음  
    무엇인가를 어디에서나 접근 가능한 전역으로 만들면 생기는 비용  

- 접근이 특정 클래스에 제한되면
  - 커플링을 제어할 수 있음
  - 중복 작업을 해야 할 수 있다는 단점이 있음

서비스가 게임의 특정 분야에 한정되어 있다면 하나의 클래스로 접근 범위를 좁히는 것도 좋음  
예를 들어 네트워크에 접근하기 위한 서비스는 온라인 클래스에서만 사용하게 하고,  
로그같이 훨씬 다양한 곳에서 사용하는 서비스는 전역에 둠
...
이전보다 데이터 연산은 훨씬 빨라졌지만, 데이터를 가져오는 건 그다지 빨라지지 않았음  

![image](https://user-images.githubusercontent.com/32252062/75650193-97377580-5c98-11ea-8378-14d92b72e130.png)
(그림 17-1) 1980년대를 기준으로 봤을 때 프로세서와 RAM 속도의 변화

CPU 속도 증가를 RAM이 따라가지 못하자 하드웨어 개발자들은 CPU캐싱 이라는 다른 해결책을 발견함  
요즘 CPU 안에는 작은 메모리가 따로 들어 있음  
이 메모리는 주메모리보다 훨씬 빠르게 CPU에 데이터를 전달할 수 있음  
이 메모리의 용량이 작은 이유는 칩 안에 집어넣어야 하기 때문이면서, 훨씬 빠르면서 동시에 비싼 메모리 소자(SRAM)를 사용하기 때문  

이런 작은 크기의 메모리를 **캐시**(칩 안에 들어있는건 L1 캐시)라고 함  
칩이 RAM으로부터 데이털들 한 바이트라도 가져와야할 경우, RAM은 연속된 메모리를 선택해 캐시에 복사함  
이런 메모리 덩어리를 **캐시 라인** 이라고 함  

![image](https://user-images.githubusercontent.com/32252062/75650958-c18a3280-5c9a-11ea-81ab-00f02f89b59d.png)
(그림 17-2) 1바이트 데이터와 그 데이터가 들어 있는 캐시 라인  

다음에 필요한 데이터가 캐시 라인 안에 들어 있다면 CPU는 RAM보다 **훨씬** 빠른 캐시로부터 데이터를 바로 가져옴
캐시에서 원하는 데이터를 찾는 것을 **캐시 히트**, 데이터를 찾지 못해서 주메모리에서 데이터를 가져오는 것을 **캐시 미스**라고 함  

캐시 미스가 발생하면 CPU는 멈춤  
필요한 데이터가 없기 때문에 다음 명령어를 진행할 수가 없어서 데이터가 도착할 때까지 수백 사이클을 기다려야함  
이렇게 기다리는 시간을 최대한 피해야함, 다음과 같은 코드를 최적화한다고 해보면  
```
...
```
의미없고 비용도 비싼 sleepFor500Cycles()부터 없애야함  
캐시 미스의 성능 비용이 sleepFor500Cycles()와 같음  
매번 주메모리에서 데이터를 꺼낼 때마다 코드에 지연 함수를 추가해 넣는 것과 다를 바 없음  

#### 데이터 == 성능?
게임과 비슷한 프로그램을 캐시 라인을 최적으로 사용할 때와 최악으로 사용할 때로 나누어 만들었을 때  
캐시를 뒤엎는(캐시 무효화가 계속 반복되는 현상)코드를 벤치마킹해서 문제가 어느 정도인지 테스트 했음  

두 프로그램은 캐시 미스가 발생하는 횟수만 다를 뿐 하는 일은 오나전히 같았음에도 속도가 50배나 차이가 났음  

성능을 데이터가 아닌 코드 측면에서만 볼 것이 아니라 캐시 때문에 **데이터를 어떻게 두느냐가 성능에 직접적인 영향을 미침**  

칩이 어떤 데이터를 읽을 때마다 캐시 라인을 같이 읽어옴  
캐시 라인에 있는 값을 더 만힝 사용할수록 더 빠르게 만들 수 있음  
즉, **자료구조를 잘 만들어서 처리하려는 값이 메모리 내에서 서로 가까이 붙어 있도록 하는 것**이 목표  
(여기에서는 스레드가 하나라는 가정, 만약 멀티스레드 어플리케이션에서 근처 데이터를 고쳐야 한다면, 코어마다 다른 캐시 라인에 데이터를 두는 게 빠름, 두 스레드에서 같은 캐시 라인에 들어 있는 데이터를 고치려 든다면 두 코어 모두 비싼 캐시 동기화 작업을 다시 해야함)  

다시 말해서 코드가 '이것', '저것', '그것'을 순서로 처리해야 한다면 다음과 같이 배치하는 게 좋음  
![image](https://user-images.githubusercontent.com/32252062/75652317-1a0eff00-5c9e-11ea-953f-ff38b9d11462.png)
(그림 17-3) 세 객체가 메모리에 서로 나란히 붙어 있음  

'이것', '저것', '그것'이 **포인터**가 아니라는 점에 주의  
실제 객체 데이터가 순서대로 들어있음  
CPU는 '이것'을 읽으면서, '저것'과 '그것'도 같이 얻음  

#### 17.3 패턴
요즘 CPU는 **메모리 접근 속도를 높이기 위한 캐시**를 여러 개 둠  
캐시를 사용하면 **최근에 접근한 메모리 근처에 있는 메모리를 훨씬 빠르게** 접근할 수 있음  
**데이터 지역성을 높일수록**, 즉, **데이터를 처리하는 순서대로 연속된 메모리에 둘수록** 캐시를 통해서 성능을 향상할 수 있음  

#### 17.4 언제 쓸 것인가?  
다른 최적화 기법과 마찬가지로, 데이터 지역성 패턴은 **성능 문제가 있을 때** 써야 함  
거의 실행되지 않는 코드에 이 패턴을 적용하겠다고 시간을 낭비하지 말자  
필요 없는 곳을 최적화 해 봐야 코드만 복잡해지고 유연성만 떨어짐  

특히 데이터 지역성 패턴은 성능 문제가 **캐시 미스** 때문인지를 확인해야 함  
다른 이유 때문이라면 데이터 지역성 패턴이 전혀 도움이 안됨  

보통 간단하게 프로파일링할 때에는 코드 두 지점 사이에 얼마나 시간이 지났는지를 직접 타이머 코드를 넣어서 확인함  
하지만 캐시 사용량을 확인하려면 좀 더 복잡한 방법이 필요  
제대로 하려면 캐시 미스가 얼마나 많이 발생했는지, 어디에서 발생했는지를 알 수 있어야함  

다행히 캐시 미스를 분석할 수 있는 프로파일러들이 이미 나와 있음  
대대적으로 자료구조를 뜯어고치기 전에 먼저 프로파일러를 하나 선택해 실행해보고 프로파일러가 뱉어내는 숫자가 무엇을 의미하는지 먼저 이해하는 게 좋음  

분명히 캐시 미스는 게임 성능에 영향을 미침  
미리부터 캐시 최적화를 하겠다고 많은 시간을 낭비해서는 안 되겠지만,  
개발하는 내내 자료구조를 캐시하기 좋게 만들려고 노력할 필요는 있음  

#### 17.5 주의사항  
소프트웨어 아키텍처의 전형적인 특징 하나가 추상화  
객체지향 언어에서는 거의 언제나 인터페이스를 사용해 디커플링함  

C++에서 인터페이스를 사용하려면 포인터나 레퍼런스를 통해 객체에 접근해야함  
하지만 포인터를 쓰게 되면 메모리를 여기저기 찾아가야 하기 때문에 데이터 지역성 패턴을 통해서 피하고자 했떤 캐시 미스가 발생함  

데이터 지역성 패턴을 위해서는 추상화의 일부 희생해야 함  
데이터 지역성에 맞춰서 구조를 잡을수록 상속, 인터페이스와 이로부터 얻을 수 있는 이득을 포기해야 함  
고민해가면서 절충하는 것 외에 정답은 따로 없음  

#### 17.6 예제 코드
데이터를 조직하는 가장 일반적인 방법 몇 개  

#### 연속 배열
많은 게임 개체를 다루는 게임 루프부터 시작해보자  
게임 개체는 컴포넌트 패턴을 이용해 AI, 물리, 렌더링과 같은 분야로 나뉨  
GameEntity 클래스는 다음과 같음
```
class GameEntity
{
public:
...
각 컴포넌트에는 벡터 몇 개 또는 행렬 한 개 같은 몇몇 상태와 이를 업데이트하기 위한 메서드가 들어 있음  

```
class AIComponent
{
public:
  void update() { /* 작업 후 상태를 변경 */ }
...
매번 게임 루프를 돌 때마다 다음과 같은 작업을 함

1. 모든 개체의 AI 컴포넌트를 업데이트
2. 모든 개체의 물리 컴포넌트를 업데이트
3. 렌더링 컴포넌트를 통해서 모든 개체를 렌더링

많은 게임 코드가 이를 다음과 같이 구현
```
while (!gameOver)
{
  // AI를 처리
  ...
  // 그 외 게임 루프에서 실행해야 할 작업들
}
```

CPU 캐시를 알기 전에는 이 코드가 전혀 문제 없어 보임  
하지만 이 코드는 좋은 코드가 아님  
무슨 일이 벌어지는지 보면  

1. 게임 개체가 배열에 **포인터**로 저장되어 있어서, 배열 값에 접근할 때마다 포인터를 따라가면서 캐시 미스가 발생
2. 게임 개체는 컴포넌트를 포인터로 들고 있어서 다시 한 번 캐시 미스가 발생
3. 컴포넌트를 업데이트
4. **모든 개체의 모든 컴포넌트**에 대해서 같은 작업을 반복

무서운 점은 이들 객체가 메모리에 어떻게 배치될지를 메모리 관리자만 알고 우리는 전혀 모른다는 점  
개체가 할당, 해제를 반복할수록 힙은 점점 어질러짐

![image](https://user-images.githubusercontent.com/32252062/75659130-d243a400-5cac-11ea-895d-b1e167f8a4c0.png)
(그림 17-4) 매 프레임 게임 루프가 실행될 대마다 필요한 데이터를 찾기 위해 이들 화살표를 전부 따라다녀야함

포인터를 따라다니느라 시간을 낭비하는 것을 '포인터 추적'이라고 부름, 전혀 좋은 게 아님  

포인터를 통해 게임 개체에 접근하는 유일한 이유가 게임 개체에 있는 또 다른 포인터를 통해 컴포넌트에 곧바로 접근하기 위해서라는 게 먼저 눈에 띔  
GameEntity 클래스에는 흥미로운 상태나 유용한 메서드가 전혀 없음  
게임 루프에는 **컴포넌트**만 있으면 됨
다음과 같이 AI, 물리, 렌더링 컴포넌트 자료형별로 큰 배열을 준비
```
...
배열에 컴포넌트 포인터가 아닌 컴포넌트 객체가 들어간다는 점이 중요  
모든 데이터가 배열 안에 나란히 들어 있기 때문에 게임 루프에서는 객체에 바로 접근할 수 있음
...
포인터 추적을 전부 제거했음  
메모리를 여기저기 뒤지지 않고 연속된 배열 세 개를 쭉 따라갈수 있게 되었음

![image](https://user-images.githubusercontent.com/32252062/75661522-d376d000-5cb0-11ea-9395-9fcbe998d65e.png)
(그림 17-5) 단일 자료형 컴포넌트 배열

바뀐 코드는 쭉 이어져 있는 바이트 스트림을 CPU에 계속 밀어 넣음  
캡슐화를 많이 깨먹지 않았다는 점도 흥미로움  

물론 게임 루프에서 게임 개체를 통하지 않고 컴포넌트를 직접 업데이트하기는 함  
하지만 그 전에도 게임 루프는 컴포넌트들을 AI, 물리, 렌더링 순서로 업데이트하기 위해서 entities[i]->ai()->update(); 같은 식으로 컴포넌트에 직접 접근하고 있었음  
컴포넌트는 자기 데이터와 메서드가 있다는 점에서 여전히 잘 캡슐화되어 있음  
단순히 컴포넌트가 사용되는 방식만 바꿨을 뿐  

GameEntity 클래스를 없앨 필요는 없음  
GameEntity가 자기 컴포넌트에 대한 포인터를 가지도록 만들 수 있음  
이때 포인터는 이들 배열에 있는 객체를 가리킴  
개념적인 '게임 개체'와 개체에 포함되어 있는 것 전부를 한 객체로 다른 코드에 전달할 수 있다는 점에서 게임 개체는 여전히 쓸모가 있음  
게임 루프가 성능에 민감하기 때문에 게임 개체를 우회해서 게임 개체 내부 데이터(컴포넌트)에 직접 접근했다는 점이 중요

#### 정렬된 데이터
이번엔 파티클 시스템을 살펴보자  
앞에서 본 것처럼 모든 파티클을 하나의 연속적인 배열에 두고, 이를 간단한 관리 클래스로 래핑  
```
class Particle
{
public:
